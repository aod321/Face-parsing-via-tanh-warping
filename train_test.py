...
class TrainModel(TemplateModel):
    ...
    def train(self):
        ...
        for batch in tqdm(self.train_loader):
            ...
            loss_out = self.train_loss(batch)
            # loss_in.backward()
            loss_out.backward()
            self.optimizer.step()
            ...
            if self.step % self.display_freq == 0:
                ...
                print('epoch {}\tstep {}\tloss_out{:.3}'.format(self.epoch, self.step,
                                                                loss_out.item()))
    ...
    def eval(self):
        self.model.eval()
        with torch.no_grad():
            _, error_out = self.eval_error()
        if os.path.exists(self.ckpt_dir) is False:
            os.makedirs(self.ckpt_dir)

        if error_out < self.best_error:
            self.best_error = error_out
        ...
        self.writer.add_scalar('eval_error%s' % uuid, error_out, self.epoch)
        print(
            'epoch {}\t error_out {:.3} best_error {:.3}'.format(self.epoch, error_out,
                                                                 self.best_error))
        with torch.cuda.device(args.cuda):
            torch.cuda.empty_cache()
